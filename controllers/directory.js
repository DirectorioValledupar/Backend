...
const CrearDirectory = async( req, res = response) => {
    ...
    try {
        const {name, startHour, endHour, lat, lng, model, img, numberContact, numberWhatsApp, time} = req.body;
        ...
        if(startHour == '' || startHour == null || startHour == undefined){
            return res.status(200).json({
                header: [{
                    error:`Por favor ingrese el tiempo disponible`,
                    code: 400,
                }],
                body:[{}]
            })
        ...
        if(endHour == '' || endHour == null || endHour == undefined){ 
            return res.status(200).json({
                header: [{
                    error:`Por favor ingrese el tiempo disponible`,
                    code: 400,
